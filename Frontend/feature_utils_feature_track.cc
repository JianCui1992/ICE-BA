    ...
    std::vector<cv::KeyPoint> *kp_in_large_img_ptr, cv::Mat_<uchar> *mask_out)
{
    ...
    for (const auto &kp : kp_in_large_img)
    {
        ...
        {
            for (int j = x - det_half_mask_size; j <= x + det_half_mask_size;
                 ++j)
            {
                (*mask_out)(i, j) = 0x00;
            }
                              ...
                              int *feat_min_num_thres, float *grid_occ_ratio)
{
    ...
    for (const cv::KeyPoint &kp : keypoints)
    {
        ...
        if (c == xbins)
            --c; // Take care of the remainder bins
        if (r == ybins)
            --r;
        grid_occ(r, c)++;
    }
    ...
    // suppression is too aggressive.  grid_occ_threshold can go to zero.  In
    // that case,
    ...
    if (dists_sorted.back() > threshold)
    {
        for (size_t i = 0; i < status->size(); ++i)
        {
            if ((*status)[i] && dists[i] > threshold)
            {
                ...
                {
                    LOG(ERROR)
                        << "filter ft_id: " << pre_small_img_kp[i].class_id
                        << " flow_dist = " << dists[i];
                }
            }
        }
    ...
    const bool absolute_static)
{
    ...
    propagate_with_optical_flow(
        ...
        mask_with_of_out_ptr, orb_feat_OF_ptr, init_pixel_shift, K_ptr,
    ...
    const bool absolute_static)
{
    ...
    int t_of_orb_us = 0;
    int t_of_total_us = 0;
    MicrosecondTimer of_total_timer("propagate_with_optical_flow total time",
                                    2);
    ...
    if (pre_image_keypoints.size() > 0)
    {
        ...
        {
            // TODO(mingyu): Use vio project & backProject (faster / more
            // accurate)
            cv::Matx33f K = *K_ptr;
            ...
            {
                cv::Vec3f pre_ray(pre_feat_undistorted[i].x,
                                  pre_feat_undistorted[i].y, 1);
            ...
            CHECK_EQ(pre_image_keypoints_small.size(),
                     pre_image_pt_rotated.size());
        } else
        {
        ...
        for (size_t i = 0; i < pre_image_keypoints_small.size(); i++)
        {
            if (pre_image_keypoints_small[i].response > 1e-7 &&
                ...
                pre_image_keypoints_small[i].pt.x <
                    img_in_smooth_small.cols - 1 &&
                pre_image_keypoints_small[i].pt.y <
                    img_in_smooth_small.rows - 1)
            {
                pre_image_strong_pt_small.push_back(
                    pre_image_keypoints_small[i].pt);
                pre_image_strong_kp_small.push_back(
                    pre_image_keypoints_small[i]);
                ...
                if (VLOG_IS_ON(3))
                {
                    if (pre_image_keypoints_small[i].response <= 1e-7)
                    {
                        LOG(ERROR) << "pre_image_kpts_small[" << i << "] id = "
                                   << pre_image_keypoints_small[i].class_id
                                   << " has weak response: "
    ...
    if (pre_image_strong_feature_ids.size() > 0)
    {
        ...
        float of_prop_ratio =
            static_cast<float>(of_feat_num_out) / of_feat_num_in;
        ...
        if (redo_of)
        {
            ...
            sampleBrightnessHistogram(img_in_smooth_pyramids.at(0), &hist_cur,
                                      &avg_cur);
            sampleBrightnessHistogram(pre_image_pyramids.at(0), &hist_pre,
                                      &avg_pre);
            float pre_scale = matchingHistogram(
                hist_pre, hist_cur, static_cast<float>(avg_cur) / avg_pre);

            // [NOTE] cv::Mat_<uchar> automatically handles the clipping of
            // uchar
            std::vector<cv::Mat> pre_image_pyramids_clone(
                pre_image_pyramids.size());
...
#ifndef USE_OPENCV_OPTICAL_FLOW
            XP_OPTICAL_FLOW::XPcalcOpticalFlowPyrLK(
                pre_image_pyramids_clone, img_in_smooth_pyramids,
                &pre_xp_kp_small, &cur_small_img_pt, &status, &err, kWinSize,
                kMaxLevel, kStartLevel, criteria, cv::OPTFLOW_USE_INITIAL_FLOW);
#else
            cv::calcOpticalFlowPyrLK(
                pre_image_pyramids_clone.at(1), img_in_smooth_small,
                pre_image_strong_pt_small, cur_small_img_pt, status, err,
                kWinSize,
                kMaxLevel -
                    1, // cv OpticalFlow starts from pyr1 (pre_image_small)
                criteria,
        ...
        // 2) OF features may cluster together and hence the feature
        ...
        for (size_t i = 0; i < status.size(); i++)
        {
            ...
            {
                // Need to preserve margin for computing orb descriptor (20
                // pixels at pyramid 0)
        ...
        if (true)
        {
            ...
            suppress_sort_kp_in_larger_img(img_in_smooth, mask,
                                           cur_small_img_kp, key_pnts_ptr,
                                           mask_with_of_out_ptr);
        } else
        {
            // Do not refine corner location.  Do not suppress OF features,
            // either.
        ...
        // Update active feature tracks of this frame.  Also perform the random
        ...
        if (VLOG_IS_ON(2))
        {
            ...
            {
                cv::circle(img_color, (*key_pnts_ptr)[i].pt, 2,
                           cv::Vec3b(0, 255, 255));
            }
        ...
        if (orb_feat_OF_ptr != nullptr)
        {
            ...
            if (key_pnts_ptr->size() > 0)
            {
                ...
                ORBextractor::computeDescriptorsN512(
                    img_in_smooth, *key_pnts_ptr, orb_feat_OF_ptr);
#else
    ...
    std::vector<cv::KeyPoint> *key_pnts)
{
    for (cv::KeyPoint &kp : *key_pnts)
    {
        ...
        // Random drop out and re-assign to a new one if this feature track is
        // too long.
        ...
        {
            kp.class_id =
                id_generator_.get(); // Re-assign a new feature track id
            feature_tracks_map_.emplace(kp.class_id, FeatureTrack(kp.pt));
#ifndef __FEATURE_UTILS_NO_DEBUG__
            VLOG(2) << " insert ftId = " << kp.class_id << " dropout "
                    << ft_it->first << " with length " << ft_it->second.length
                    << "!";
#endif
        } else
        {
...
// Determine if we need to detect new feature
    ...
    // or
    // 2) Features are too few: depending on the threshold for grid_occ_ratio
    // and request_feat_num,
    ...
    if (re_detect)
    {
        ...
        // feature tracks
        // [NOTE] In the rare case re_detect is triggered and
        // propagated_feat_num == request_feat_num,
        // (feature distribution is too concentrated), we enforce to detect up
        // to 10 more features.
...
#ifndef __FEATURE_UTILS_NO_DEBUG__
            CHECK_EQ(orb_feat_ptr->at<uchar>(0, 0),
                     orb_feat_OF.at<uchar>(0, 0));
#endif
        }
    ...
    if (1)
    {
        ...
        cv::putText(img_color,
                    "redet feats: " +
                        boost::lexical_cast<std::string>(orb_feat_new.rows),
                    cv::Point2i(10, 35), cv::FONT_HERSHEY_SIMPLEX, .5,
                    cv::Scalar(0, 0, 255), 1);
        cv::putText(img_color,
                    "Total feat# " +
                        boost::lexical_cast<std::string>(key_pnts_ptr->size()),
