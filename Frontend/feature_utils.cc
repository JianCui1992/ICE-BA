...
void build_pyramids(const cv::Mat &img, int max_level,
                    std::vector<cv::Mat> *_pyramids, uchar *const pyra_buf_ptr)
{
    ...
    } else
    {
        ...
        for (int lvl = 0; lvl <= max_level; ++lvl)
        {
            ...
            {
                cv::Mat tmp(img.rows, img.cols, img.type(),
                            pyra_buf_ptr + offset);
                img.copyTo(tmp);
                pyramids[lvl] = tmp;
            }
...
inline cv::Mat fast_mask_pyra_down(const cv::Mat &mask)
{
    ...
    cv::Mat mask_small(mask.rows / compress_ratio, mask.cols / compress_ratio,
                       CV_8U);
    ...
    for (int y = 0; y < mask_small.rows; y++)
    {
        for (int x = 0; x < mask_small.cols; x++)
        {
            ...
            const int shift0 =
                (y * compress_ratio) * width_step_in + x * compress_ratio;
...
// 1. refine_kp_in_larger_img takes the keypoints in the small image (higher
// pyramid level),
//    and search in a local region in the large image (lower pyramid level).
//    The refined
//    keypoint location is computed as the weighted average of local points by
//    harris response.
// 2. The response of the refined keypoint is passed from the response from the
// original detection.
//    May NOT be harris response though.
inline bool
refine_kp_in_larger_img(const cv::Mat &img_in_smooth,
                        ...
                        std::vector<cv::KeyPoint> *refined_kp_in_large_img_ptr)
{
    ...
    std::vector<cv::KeyPoint> &refined_kp_in_large_img =
        *refined_kp_in_large_img_ptr;
    ...
    for (int kp_small_idx = 0; kp_small_idx < kp_in_small_img.size();
         ++kp_small_idx)
    {
        ...
        {
            for (int x =
                     key_pnt_small.pt.x * compress_ratio - compress_ratio + 1;
        ...
        // If the local keypoints (in large image) are NOT empty, we compute the
        // weighted average
        // of the point location and response.
        if (local_kps.size() > 0)
        {
            ORBextractor::HarrisResponses(img_in_smooth, harris_block_size,
                                          0.04f, &local_kps);
            float score_total = 0;
            ...
            for (size_t i = 0; i < local_kps.size(); i++)
            {
                ...
                VLOG(3) << "local_kp.response " << local_kps[i].response
                        << " local_kp.pt " << local_kps[i].pt << " score_total "
                        << score_total;
#endif
            }
            ...
            cv::KeyPoint best_local_kp =
                local_kps[best_local_kp_id]; // copy scale score etc.
...
// Then take the average of the farthest ray that is still projected inside the
// image
// as the cropping FOV.
// However, we limit the FOV upper bound to 160 degrees (for fisheye case).
bool generate_cam_mask(const cv::Matx33f &K, const cv::Mat_<float> &dist_coeffs,
                       const cv::Size &mask_size, cv::Mat_<uchar> *cam_mask,
                       float *fov_deg)
{
    ...
    for (int i = 0 ; i < 2; ++i)
    {
        ...
        for (int deg = 30; deg < 90; deg += 5)
        {
            ...
            cv::projectPoints(ray, cv::Vec3f(), cv::Vec3f(), K, dist_coeffs,
                              dist_pt);

            if (dist_pt[0].x > 0 && dist_pt[0].y > 0 &&
...
bool detect_orb_features(
    ...
    FeatureTrackDetector *feat_track_detector, float refine_harris_threshold)
{
    CHECK_GT(pyra_level, 0);
    // Only detect feature at det_pyra_level, and then look for refined corner
    // position at level 0.
    // For now, all the features are *refined* to pyramid0 as octave is 0 for
    // all detected features
    // ORBextractor runs detection at det_pyra_level with levels orb_pyra_levels
    // = 1, which means
    // no multi-pyramids within ORBextractor.
    // TODO(mingyu): Pre-compute the pyramids if needed to avoid duplicate
    // computation when calling
    ...
    float more_points_ratio =
        1.5; // because refinement may reduce points number
    ...
    constexpr int orb_pyra_levels =
        1; // The pyramid levels used in ORBextractor
    // [NOTE] We need the score (keypoint.response) computed as harris score
    // instead
    //        of FAST.  It's slower but more discriminative to sort / refine
    //        keypoints.
    ORBextractor orb(request_feat_num * more_points_ratio, 2, orb_pyra_levels,
                     ORBextractor::HARRIS_SCORE, fast_thresh, use_fast);
    orb.detect(img_pyramids[det_pyra_level], mask_pyramids[det_pyra_level],
    ...
    VLOG(1) << "Before uniformaty check ORBextractor gets "
            ...
            << " level = " << det_pyra_level << " mask_pyramids["
            << det_pyra_level << "].size "
    ...
    if (mask_pyramids[det_pyra_level].rows > 0)
    {
        for (auto &kp : kp_in_pyramids[det_pyra_level])
        {
            ...
            {
                LOG(FATAL) << "mask_pyramids[" << det_pyra_level << "]" << kp.pt
                           << " = 0x00";
            }
        }
    }
        ...
        kp_in_pyramids[det_pyra_level].size() > 1)
    {
        ...
        // Basically, this weight_LUT can suppress at most a radius of 15
        // pixels.  In pracice,
        // it is possible to see features that are 2 to 4 pixels apart without
        // being suppressed.
        ...
        for (int x = 0; x < 2 * 16 - 1; ++x)
        {
            for (int y = 0; y < 2 * 16 - 1; ++y)
            {
                weight_LUT.at<float>(y, x) =
                    std::max(1 -
                                 static_cast<float>((15 - x) * (15 - x) +
                                                    (15 - y) * (15 - y)) /
                                     static_cast<float>(15 * 15),
                             0.f);
            }
        }
        ...
        // TODO(mingyu): Implement a simple minded binary mask instead of using
        // weighted mask
        // TODO(mingyu): Rewrite XpEnforceKeyPointUniformity to take vector of
        // cv::KeyPoint directly
        ...
        if (orb_feat_ptr != nullptr)
        {
            ...
            ORBextractor::computeOrientation(img_pyramids[det_pyra_level], umax,
                                             &kp_in_pyramids[det_pyra_level]);
        }
    }
#ifndef __FEATURE_UTILS_NO_DEBUG__
    ...
    if (VLOG_IS_ON(4))
    {
        ...
        for (const auto &kp : kp_in_pyramids[det_pyra_level])
        {
            ...
            VLOG(2) << "kp " << kp.pt << " at level " << det_pyra_level
                    << " score " << kp.response;
        }
        cv::imwrite("/tmp/img_level_" +
                        boost::lexical_cast<std::string>(det_pyra_level) +
                        ".png",
                    small_debug);
    }
#endif
    // Refine the corner response.  Push keypoints from higher pyramids to
    // pyramid 0.
    ...
    // [NOTE] This operation IGNORES and OVERWRITES existing keypoints in lower
    // pyramids if any!
    for (int it_pyra = det_pyra_level; it_pyra > 0; it_pyra--)
    {
        ...
        CHECK_GE(kp_in_pyramids[it_pyra].size(),
                 kp_in_pyramids[it_pyra - 1].size());
#ifndef __FEATURE_UTILS_NO_DEBUG__
        if (VLOG_IS_ON(4))
        {
            ...
            cv::imwrite("/tmp/img_level_" +
                            boost::lexical_cast<std::string>(it_pyra - 1) +
                            ".png",
                        small_debug);
        }
#endif
    ...
    // The original pattern bit after rotation can exceed half-window size(16),
    // 17, or 18.
    // We set feat_half_size to 20 here to keep the KeyPoint away from
    // the possible dangerous place (see the code below) before extracting ORB
    // descriptors.
    // The feat_half_size is large enough to satisfy the harris margin even
    // after moving up
    ...
    {
        if (kp.pt.x > feat_half_size && kp.pt.y > feat_half_size &&
            kp.pt.x < img_in_smooth.cols - feat_half_size &&
            kp.pt.y < img_in_smooth.rows - feat_half_size)
        {
            within_bound_kps.push_back(kp);
            within_bound_kps_small.push_back(
                cv::KeyPoint(kp.pt / 2, kp.size)); // default response is 0
        }
    }
    CHECK_EQ(within_bound_kps.size(), within_bound_kps_small.size());

    // Check the harris response at pyramid1 (match the behavior in
    ...
    // TODO(mingyu): Refactor the code to re-use the harris response computed
    // earlier.
    if (refine_harris_threshold > 0)
    {
        cv::Mat img_in_smooth_small = (pyra_level == 1)
                                          ? fast_pyra_down(img_pyramids[0])
                                          : img_pyramids[1];
        ORBextractor::HarrisResponses(img_in_smooth_small, 7, 0.04f,
                                      &within_bound_kps_small);
    } else
    {
    ...
    } else
    {
        ...
        // TODO(mingyu): This sort is dummy as XpEnforceUniformity has already
        // sorted.
        std::sort(within_bound_kps.begin(), within_bound_kps.end(),
                  internal::kp_compare);
    }
...
#endif
    for (int i = 0, count = 0;
         count < request_feat_num && i < within_bound_kps.size(); ++i)
    {
        if (within_bound_kps_small[i].response < refine_harris_threshold)
        {
#ifndef __FEATURE_UTILS_NO_DEBUG__
            VLOG(2) << "within_bound[" << i
...
#ifndef __FEATURE_UTILS_NO_DEBUG__
        VLOG(2) << "kps[" << count << "] = within_bound[" << i
                << "] id = " << kp.class_id
                << " response: " << within_bound_kps_small[i].response;
#endif

        // TODO(mingyu): use 0 as we have refined to pyra0 or use the real
        // octave at detection
        kp.octave = 0;
...
bool detect_harris_features(const cv::Mat &img_in_smooth,
                            const cv::Mat_<uchar> mask, int request_feat_num,
                            int pyra_level, int fast_thresh,
...
SlaveImgFeatureDetector::SlaveImgFeatureDetector(
    int block_size, DetectSlaveFeatureType method,
    float min_feature_distance_over_baseline_ratio,
    float max_feature_distance_over_baseline_ratio)
    : block_size_(block_size), half_block_size_(block_size / 2),
      method_(method), min_feature_distance_over_baseline_ratio_(
          ...
          max_feature_distance_over_baseline_ratio)
{
    ...
    for (int v = -half_block_size_; v < half_block_size_; ++v)
    {
        for (int u = -half_block_size_; u < half_block_size_; ++u)
        {
            ...
            gaussion_weights_[(v + half_block_size_) * block_size + u +
                              half_block_size_] = w;
...
bool SlaveImgFeatureDetector::detect_features_on_slave_img_helper(
    const cv::Mat &master_image, const cv::Mat &slave_image,
    const DuoCalibParam &duo_calib_param, int max_pixel_val_diff, int master_x,
    int master_y, const cv::Mat_<float> &s_R_m, const cv::Mat_<float> &s_t_m,
    const cv::Mat_<float> &pnt_master, const cv::Mat_<float> &search_range,
#ifdef DEBUG_DETECT_FEATURES_ON_SLAVE_IMG
    cv::Mat *slave_image_debug_ptr,
#endif
    int *min_patch_diff2_ptr, int *second_min_patch_diff2_ptr,
    int *best_slave_x_ptr, int *best_slave_y_ptr, int *best_search_dist_id_ptr,
    int *second_best_search_dist_id_ptr)
{
    const int scale = 2;
    // since we compute diff in pyr level 1, we use block_size_ rather than
    // half_block_size_ as radius
    // orb desc needs 16 pixels margin
    const int img_margin =
        (half_block_size_ * scale) > 16 ? (half_block_size_ * scale) : 16;
    // since we don't know where this point is, try to move it to the furthest
    // dis
    ...
    cv::projectPoints(pnts_slave, cv::Matx31d(), cv::Matx31d(),
                      ...
                      duo_calib_param.Camera.cv_dist_coeff_lr[1], pixels_slave);
#ifndef __FEATURE_UTILS_NO_DEBUG__
    ...
    // so second_min_patch_val_diff2 will be assigned to this val for the first
    // time
    ...
    for (int v = -half_block_size_; v < half_block_size_; ++v)
    {
        ...
        {
            const float w =
                gaussion_weights_[(v + half_block_size_) * block_size_ + u +
                                  half_block_size_];
            master_sum +=
                w * static_cast<float>(
                        master_image.data[(master_y + v * scale) * master_col +
                                          master_x + u * scale]);
        }
    }
    float master_avg = master_sum / gaussion_weight_sum_;
#ifndef __FEATURE_UTILS_NO_DEBUG__
    VLOG(2) << "master_avg " << master_avg << " max_pixel_val_diff "
            << max_pixel_val_diff;
#endif
    int best_slave_x = 0;
    int best_slave_y = 0;
    ...
    // during fine search stage, there may be sample points having the same xy.
    // Quickly jump over
    ...
    for (int it_slave = 0; it_slave < pixels_slave.rows; ++it_slave)
    {
            ...
            y < slave_image.rows - img_margin)
        {
            ...
            for (int v = -half_block_size_; v < half_block_size_; ++v)
            {
                ...
                {
                    const float w =
                        gaussion_weights_[(v + half_block_size_) * block_size_ +
                                          u + half_block_size_];
                    slave_sum +=
                        w * static_cast<int>(
                                slave_image.data[(y + v * scale) * slave_col +
                                                 x + u * scale]);
                }
            }
            ...
            VLOG(2) << "it_slave " << it_slave << " slave_avg " << slave_avg
                    << " slave x y [" << x << ", " << y << "]";
#endif
            // allow avg value diff 2x max pixel diff since the exposure of left
            // and right cam
            ...
            for (int v = -half_block_size_; v < half_block_size_; ++v)
            {
                for (int u = -half_block_size_; u < half_block_size_; ++u)
                {
                    int slave_val = static_cast<int>(
                        slave_image
                            .data[(y + v * scale) * slave_col + x + u * scale]);
                    int master_val = static_cast<int>(
                        master_image.data[(master_y + v * scale) * master_col +
                                          master_x + u * scale]);
                    int diff =
                        (master_val - master_avg) - (slave_val - slave_avg);
                    const float w =
                        gaussion_weights_[(v + half_block_size_) * block_size_ +
                                          u + half_block_size_];
...
#ifndef __FEATURE_UTILS_NO_DEBUG__
    VLOG(2) << "  avg_diff_count " << avg_diff_count << " skip_count "
            << skip_count << " early_break_count " << early_break_count
            << " candidate_count " << candidate_count << " find_count "
            << find_count;
#endif
...
bool SlaveImgFeatureDetector::detect_features_on_slave_img(
    ...
    std::vector<cv::KeyPoint> *slave_kps_ptr, cv::Mat *slave_orb_feat_ptr,
    int max_pixel_val_diff)
{
    ...
    Eigen::Matrix4f s_T_m = duo_calib_param.Camera.D_T_C_lr[1].inverse() *
    ...
    // coarse search min_search_range x cam_baseline_dis -> max_search_range x
    // cam_baseline_dis
    // Note: if min_search_range < 3, it greatly increases the search range in
    // image
    ...
    // in the inverse depth space, uniformally generate [min_range, max_range]
    // search range
    // i = 0 -> min_range
    // i = (range_bin_num - 1) -> max_range
    // ask sid for the math
    const float tmp_x = (max_feature_distance_over_baseline_ratio_ /
    ...
    {
        coarse_search_range(i) = max_feature_distance_over_baseline_ratio_ *
                                 cam_baseline_dis /
    ...
    for (size_t it_master = 0; it_master < master_pnts_undistorted.size();
         ++it_master)
    {
        const int master_x = std::roundf(master_pnts[it_master].x + 0.5);
        const int master_y = std::roundf(master_pnts[it_master].y + 0.5);
        if (master_x < half_block_size_ || master_y < half_block_size_ ||
        ...
        int second_best_search_dist_id = -1;
#ifndef __FEATURE_UTILS_NO_DEBUG__
        VLOG(2) << "before coarse det it_master " << it_master << " kp id "
                << master_kps[it_master].class_id << " master pixel ["
                << master_x << ", " << master_y << "]";
#endif
        if (!detect_features_on_slave_img_helper(
                master_image, slave_image, duo_calib_param,
                max_pixel_val_diff * 2, // coarse
                master_x, master_y, s_R_m, s_t_m, pnt_master,
                coarse_search_range,
#ifdef DEBUG_DETECT_FEATURES_ON_SLAVE_IMG
                &slave_image_debug,
#endif
                ...
                &best_search_dist_id, &second_best_search_dist_id))
        {
#ifndef __FEATURE_UTILS_NO_DEBUG__
...
// cv::circle(slave_image_debug, cv::Point2i(best_slave_x, best_slave_y), 8,
// 255);
#endif
        // threshold test
...
#endif
            // if the sampled pos are dense, a couple of samples may get close
            // to the true position
            // So they all have low pixel diff values, which should not be
            // discouraged.
            if (second_best_search_dist_id > best_search_dist_id + 1 ||
                second_best_search_dist_id < best_search_dist_id - 1)
            {
#ifndef __FEATURE_UTILS_NO_DEBUG__
                VLOG(2) << "thresh test failed min_patch_diff2 / "
                           "second_min_patch_diff2 "
                        << min_patch_diff2 << " / " << second_min_patch_diff2
                        << " = "
                        << static_cast<float>(min_patch_diff2) /
                               second_min_patch_diff2
        ...
        float max_search_dis =
            coarse_search_range(coarse_search_range.rows - 1);
        ...
        VLOG(2) << "fine det it_master " << it_master
                ...
                << search_range_fine(1) << " .. " << search_range_fine(8) << " "
                << search_range_fine(9);
#endif
        int best_slave_x, best_slave_y;
        if (!detect_features_on_slave_img_helper(
                master_image, slave_image, duo_calib_param, max_pixel_val_diff,
                master_x, master_y, s_R_m, s_t_m, pnt_master, search_range_fine,
#ifdef DEBUG_DETECT_FEATURES_ON_SLAVE_IMG
                &slave_image_debug,
#endif
                &min_patch_diff2,
                nullptr, // second_min_patch_diff2
                &best_slave_x, &best_slave_y, nullptr /* best_search_dist_id */,
        ...
        CHECK_GE(best_search_dist_id, 0);
#ifdef DEBUG_DETECT_FEATURES_ON_SLAVE_IMG
        cv::circle(slave_image_debug, cv::Point2i(best_slave_x, best_slave_y),
                   8, 255);
        cv::putText(slave_image_debug, boost::lexical_cast<std::string>(
                                           master_kps[it_master].class_id),
...
#ifndef __FEATURE_UTILS_NO_DEBUG__
    VLOG(1) << "master kp # " << master_kps.size() << " slave kp # "
            << slave_kps_ptr->size();
#ifdef DEBUG_DETECT_FEATURES_ON_SLAVE_IMG
...
#ifdef __ARM_NEON__
        ORBextractor::computeDescriptorsN512(slave_image, *slave_kps_ptr,
                                             slave_orb_feat_ptr);
#else
        ORBextractor::computeDescriptors(slave_image, *slave_kps_ptr,
                                         slave_orb_feat_ptr);
#endif
    }
    return true;
...
vector<vector<cv::DMatch>> neon_orb_match(const cv::Mat &desc_query,
                                          ...
                                          const cv::Mat &orb_desc_training)
{
    ...
    for (int it_query = 0; it_query < desc_query.rows; ++it_query)
    {
        ...
        for (int it_orb_this_rig = 0; it_orb_this_rig < orb_desc_training.rows;
             ++it_orb_this_rig)
        {
            ...
            // orb_desc_training.row(it_orb_this_rig).ptr<unsigned char>();
            const unsigned char *b =
                orb_desc_training.data + it_orb_this_rig * 32;
...
int neon_find_close_points(float query_u, float query_v, int training_num,
                           float *training_u, float *training_v, float range_sq,
                           cv::Mat *within_range_mask_ptr)
{
    ...
    for (size_t i = 0; i <= training_num - 4; i += 4)
    {
        ...
        float32x4_t vecUres2Vres2 =
            vmlaq_f32(vmulq_f32(vecUres, vecUres), vecVres, vecVres);
        ...
        for (int j = 0; j < 4; j++) {
          LOG(ERROR)
              ...
              << " matching_mask.at<uchar>(0, i + j) " <<
        (int)(matching_mask.at<uchar>(0, i + j));
        }*/
    ...
    for (size_t i = processed_num; i < training_num; i++)
    {
        const float distance2 =
            (query_u - training_u[i]) * (query_u - training_u[i]) +
            (query_v - training_v[i]) * (query_v - training_v[i]);
...
void OpencvHarrisScoreCalculator::Get2dMaxima(
...
#ifndef __FEATURE_UTILS_NO_DEBUG__
    VLOG(1) << "OpencvHarrisScoreCalculator::Get2dMaxima points.size() "
            << points.size();
#endif
}
